...
export function About() {
  ...
  return (
    <section id="about" className="relative py-24 px-4 sm:px-6 lg:px-8 bg-[#121212] overflow-hidden">
      <BackgroundEffects />
      <motion.div className="absolute top-10 right-10 w-32 h-32 bg-[#690092]/15 rounded-full pointer-events-none" animate={{ scale: [1, 1.2, 1], rotate: [0, 180, 360] }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }} />
      <motion.div className="absolute bottom-10 left-10 w-20 h-20 bg-[#690092]/10 rounded-full pointer-events-none" animate={{ scale: [1, 1.3, 1] }} transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }} />
      ...
      <div className="max-w-4xl mx-auto" ref={ref}>
        <motion.div initial={{ opacity: 0, y: 40 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7 }} className="text-center mb-12">
          <h2 className="text-4xl sm:text-5xl font-black text-white mb-4">
            About <span className="text-[#690092]">Me</span>
          </h2>
        ...
        <motion.div initial={{ opacity: 0, y: 50 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8, delay: 0.2 }} whileHover={{ y: -4 }}
          className="bg-[#1e1e1e] rounded-3xl p-8 sm:p-10 shadow-2xl border border-[#690092]/20 relative overflow-hidden mb-6"
        >
          <div className="absolute inset-0 bg-gradient-to-br from-[#690092]/5 via-transparent to-transparent pointer-events-none rounded-3xl" />
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.7, delay: 0.4, type: "spring", stiffness: 90 }}
            className="bg-[#1e1e1e] rounded-3xl p-6 shadow-xl border border-[#690092]/20"
          >
            <div className="flex items-center gap-3 mb-5">
              <motion.div className="p-2.5 bg-[#690092] rounded-xl" whileHover={{ rotate: 15 }} transition={{ type: "spring", stiffness: 300 }}>
            ...
            <div className="flex flex-wrap gap-3">
              {softSkills.map((skill, i) => (
                <motion.button key={skill}
                  ...
                  whileHover={{ scale: 1.1, y: -3, boxShadow: "0 0 15px rgba(105,0,146,0.4)" }} whileTap={{ scale: 0.95 }}
                  onHoverStart={() => setHoveredSkill(skill)} onHoverEnd={() => setHoveredSkill(null)}
                  className={`px-4 py-2 rounded-full font-bold text-sm border-2 border-[#690092] transition-all ${hoveredSkill === skill ? "bg-[#690092] text-white" : "bg-transparent text-white/80"}`}
                >
                  {skill}
                </motion.button>
              ))}
          ...
          >
            ...
            <div className="relative">
              <motion.div className="absolute left-[9px] top-2 w-0.5 bg-[#690092]/30" initial={{ height: 0 }} animate={isInView ? { height: "calc(100% - 8px)" } : {}} transition={{ duration: 0.8, delay: 0.7 }} />
              <div className="space-y-5">
                {education.map((edu, i) => (
                  <motion.div key={i} initial={{ opacity: 0, x: 20 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.5, delay: 0.65 + i * 0.15 }} whileHover={{ x: 4 }} className="flex gap-4 items-start">
                    <motion.div className="w-5 h-5 rounded-full bg-[#690092] border-4 border-[#1e1e1e] shrink-0 mt-0.5 z-10" animate={{ scale: [1, 1.25, 1] }} transition={{ duration: 2, repeat: Infinity, delay: i * 0.6 }} />
                    <div>
                      <span className="text-xs font-black bg-[#690092]/20 text-[#9b33c4] border border-[#690092]/30 px-2.5 py-0.5 rounded-full">{edu.period}</span>
