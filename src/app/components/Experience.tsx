...
export function Experience() {
  ...
  return (
    <section id="experience" className="relative py-24 px-4 sm:px-6 lg:px-8 bg-[#121212] overflow-hidden">
      <BackgroundEffects />
      <motion.div className="absolute top-0 right-0 w-56 h-56 bg-[#8000b2]/10 rounded-full pointer-events-none" animate={{ scale: [1, 1.3, 1] }} transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }} />
      ...
      <div className="max-w-4xl mx-auto" ref={ref}>
        <motion.div initial={{ opacity: 0, y: 40 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7 }} className="text-center mb-12">
          <h2 className="text-4xl sm:text-5xl font-black text-white mb-4">
            Experience & <span className="text-[#8000b2]">Achievements</span>
          </h2>
              ...
              return (
                <motion.div key={index} initial={{ opacity: 0, x: -40 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.55, delay: 0.3 + index * 0.15, type: "spring", stiffness: 90 }} className="relative">
                  <div className="absolute left-6 top-6 hidden md:block z-10">
                    <motion.div className="w-5 h-5 bg-[#8000b2] border-4 border-[#121212] rounded-full" style={{ boxShadow: "0 0 12px rgba(128,0,178,0.6)" }} animate={{ scale: [1, 1.3, 1] }} transition={{ duration: 2, repeat: Infinity, delay: index * 0.4 }} />
                  </div>
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex flex-wrap items-start justify-between gap-2 mb-1">
                          ...
                          <span className="text-xs font-bold bg-[#8000b2]/20 text-[#a033cc] border border-[#8000b2]/30 px-2.5 py-1 rounded-full shrink-0">{exp.period}</span>
                        </div>
                        <p className="text-[#8000b2] text-sm font-semibold mb-1">{exp.organization}</p>
