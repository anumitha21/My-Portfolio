...
export function Projects() {
  ...
  return (
    <section id="projects" className="relative py-24 px-4 sm:px-6 lg:px-8 bg-[#121212] overflow-hidden">
      <BackgroundEffects />
      <motion.div className="absolute bottom-0 left-0 w-72 h-72 bg-[#8000b2]/10 rounded-full pointer-events-none" animate={{ x: [0, 40, 0], y: [0, -30, 0] }} transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }} />
      ...
      <div className="max-w-6xl mx-auto" ref={ref}>
        <motion.div initial={{ opacity: 0, y: 40 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7 }} className="text-center mb-12">
          <h2 className="text-4xl sm:text-5xl font-black text-white mb-4">
            My <span className="text-[#8000b2]">Projects</span>
          </h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project, index) => (
            <motion.div key={index}
              ...
              whileHover={{ y: -8, boxShadow: "0 24px 48px rgba(128,0,178,0.25)" }}
              className="bg-[#1e1e1e] rounded-2xl p-6 border border-[#8000b2]/20 hover:border-[#8000b2]/50 flex flex-col transition-all duration-300"
            >
              <div className="flex items-start justify-between mb-3">
                <h3 className="text-xl font-black text-white">{project.title}</h3>
                <span className="text-3xl font-black text-[#8000b2]/15 leading-none">0{index + 1}</span>
              </div>
              ...
              <div className="flex flex-wrap gap-2 mb-5">
                {project.tags.map((tag, j) => (
                  <motion.span key={j} initial={{ opacity: 0, scale: 0.8 }} animate={isInView ? { opacity: 1, scale: 1 } : {}} transition={{ delay: index * 0.15 + j * 0.04 }}
                    className="px-2.5 py-1 bg-[#8000b2]/15 border border-[#8000b2]/30 text-[#a033cc] text-xs font-bold rounded-full"
                  >{tag}</motion.span>
                ))}
              </div>
              <motion.button whileHover={{ scale: 1.04, boxShadow: "0 0 20px rgba(128,0,178,0.4)" }} whileTap={{ scale: 0.97 }}
                onClick={() => window.open(project.github, "_blank")}
                className="flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-[#8000b2] text-white font-bold text-sm transition-all"
              >
