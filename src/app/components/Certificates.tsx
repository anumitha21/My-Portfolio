...
export function Certificates() {
  ...
  return (
    <section id="certificates" className="relative py-24 px-4 sm:px-6 lg:px-8 bg-[#1a1a1a] overflow-hidden">
      <BackgroundEffects />
      <motion.div className="absolute -bottom-10 -left-10 w-60 h-60 bg-[#690092]/8 rounded-full pointer-events-none" animate={{ rotate: 360 }} transition={{ duration: 25, repeat: Infinity, ease: "linear" }} />
      ...
      <div className="max-w-4xl mx-auto" ref={ref}>
        <motion.div initial={{ opacity: 0, y: 40 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7 }} className="text-center mb-12">
          <h2 className="text-4xl sm:text-5xl font-black text-white mb-4">
            <span className="text-[#690092]">Certifications</span>
          </h2>
            ...
            {certificates.map((cert, index) => (
              <motion.div key={index} initial={{ opacity: 0, x: -40 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.55, delay: 0.3 + index * 0.15, type: "spring", stiffness: 90 }} className="relative">
                <div className="absolute left-6 top-6 hidden md:block z-10">
                  <motion.div className="w-5 h-5 bg-[#690092] border-4 border-[#1a1a1a] rounded-full" style={{ boxShadow: "0 0 12px rgba(105,0,146,0.6)" }} animate={{ scale: [1, 1.3, 1] }} transition={{ duration: 2, repeat: Infinity, delay: index * 0.5 }} />
                </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex flex-wrap items-start justify-between gap-2 mb-1">
                        ...
                        <span className="text-xs font-bold bg-[#690092]/20 text-[#9b33c4] border border-[#690092]/30 px-2.5 py-1 rounded-full shrink-0">{cert.period}</span>
                      </div>
                      <p className="text-[#690092] text-sm font-semibold mb-1">{cert.organization}</p>
